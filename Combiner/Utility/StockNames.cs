using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Combiner
{
	public static class StockNames
	{


		public static string Albatross = "Albatross";
		public static string Anaconda = "Anaconda";
		public static string Anglerfish = "Anglerfish";
		public static string Ant = "Ant";
		public static string Archerfish = "Archerfish";
		public static string Armadillo = "Armadillo";
		public static string Baboon = "Baboon";
		public static string Bat = "Bat";
		public static string Behemoth = "Behemoth";
		public static string Beluga = "Beluga";
		public static string BlackWidow = "Black Widow";
		public static string BlueRingedOctopus = "Blue Ringed Octopus";
		public static string BlueWhale = "Blue Whale";
		public static string BolasSpider = "Bolas Spider";
		public static string BombardierBeetle = "Bombardier Beetle";
		public static string Bull = "Bull";
		public static string Camel = "Camel";
		public static string Chameleon = "Chameleon";
		public static string Cheetah = "Cheetah";
		public static string Chimpanzee = "Chimpanzee";
		public static string Cockroach = "Cockroach";
		public static string Condor = "Condor";
		public static string Coyote = "Coyote";
		public static string Crocodile = "Crocodile";
		public static string Cuttlefish = "Cuttlefish";
		public static string DiamondPython = "Diamond Python";
		public static string Dolphin = "Dolphin";
		public static string Dragonfly = "Dragonfly";
		public static string Eagle = "Eagle";
		public static string ElectricEel = "Electric Eel";
		public static string Elephant = "Elephant";
		public static string Firefly = "Firefly";
		public static string GardenSpider = "Garden Spider";
		public static string Garfish = "Garfish";
		public static string GiantAnteater = "Giant Anteater";
		public static string GiantSquid = "Giant Squid";
		public static string Giraffe = "Giraffe";
		public static string Gorilla = "Gorilla";
		public static string GreatWhiteShark = "Great White Shark";
		public static string GrizzlyBear = "Grizzly Bear";
		public static string HammerheadShark = "Hammerhead Shark";
		public static string Hatchetfish = "Hatchetfish";
		public static string HerculesBeetle = "Hercules Beetle";
		public static string Hippopotamus = "Hippopotamus";
		public static string Hornbill = "Hornbill";
		public static string Hornet = "Hornet";
		public static string Horse = "Horse";
		public static string HumpbackWhale = "Humpback Whale";
		public static string Hyena = "Hyena";
		public static string Kangaroo = "Kangaroo";
		public static string KillerWhale = "Killer Whale";
		public static string KomodoDragon = "Komodo Dragon";
		public static string Lammergeier = "Lammergeier";
		public static string Lemming = "Lemming";
		public static string Lioness = "Lioness";
		public static string Lobster = "Lobster";
		public static string MalayanTapir = "Malayan Tapir";
		public static string ManOWar = "Man O' War";
		public static string MantisShrimp = "Mantis Shrimp";
		public static string Moose = "Moose";
		public static string MountainLion = "Mountain Lion";
		public static string MuskOx = "Musk Ox";
		public static string NakedMoleRat = "Naked Mole Rat";
		public static string Narwhal = "Narwhal";
		public static string Oogpister = "Oogpister";
		public static string Ostrich = "Ostrich";
		public static string Panther = "Panther";
		public static string Piranha = "Piranha";
		public static string PistolShrimp = "Pistol Shrimp";
		public static string PoisonFrog = "Poison Frog";
		public static string PolarBear = "Polar Bear";
		public static string Porcupine = "Porcupine";
		public static string PrayingMantis = "Praying Mantis";
		public static string Ram = "Ram";
		public static string Rat = "Rat";
		public static string Rattlesnake = "Rattlesnake";
		public static string Raven = "Raven";
		public static string Rhinoceros = "Rhinoceros";
		public static string Scorpion = "Scorpion";
		public static string ShieldBug = "Shield Bug";
		public static string SkipperCaterpillar = "Skipper Caterpillar";
		public static string Skunk = "Skunk";
		public static string Snail = "Snail";
		public static string SnappingTurtle = "Snapping Turtle";
		public static string SnowyOwl = "Snowy Owl";
		public static string SpermWhale = "Sperm Whale";
		public static string SpittingCobra = "Spitting Cobra";
		public static string SpittingSpider = "Spitting Spider";
		public static string Tarantula = "Tarantula";
		public static string Termite = "Termite";
		public static string Tiger = "Tiger";
		public static string BluefinTuna = "Bluefin Tuna";
		public static string VelvetWorm = "Velvet Worm";
		public static string Vulture = "Vulture";
		public static string WalkingStick = "Walking Stick";
		public static string Walrus = "Walrus";
		public static string Warthog = "Warthog";
		public static string Wasp = "Wasp";
		public static string WhaleShark = "Whale Shark";
		public static string Wolf = "Wolf";
		public static string Wolverine = "Wolverine";
		public static string WoollyMammoth = "Woolly Mammoth";
		public static string Zebra = "Zebra";
		public static string GiantPacificOctopus = "Giant Pacific Octopus";

		public static Dictionary<string, string> ProperStockNames = new Dictionary<string, string>()
		{
			{"albatross", Albatross },
			{"anaconda", Anaconda },
			{"anglerfish", Anglerfish },
			{"ant", Ant },
			{"archerfish", Archerfish },
			{"armadillo", Armadillo },
			{"baboon", Baboon },
			{"vampire_bat", Bat },
			{"behemoth", Behemoth },
			{"beluga", Beluga },
			{"black_widow", BlackWidow },
			{"blue_ringed_octopus", BlueRingedOctopus },
			{"blue_whale", BlueWhale },
			{"bolas_spider", BolasSpider },
			{"bombardier_beetle", BombardierBeetle },
			{"bull", Bull },
			{"camel", Camel },
			{"chameleon", Chameleon },
			{"cheetah", Cheetah },
			{"chimpanzee", Chimpanzee },
			{"cockroach", Cockroach },
			{"condor", Condor },
			{"coyote", Coyote },
			{"crocodile", Crocodile },
			{"cuttlefish", Cuttlefish },
			{"diamond_python", DiamondPython },
			{"dolphin", Dolphin },
			{"dragonfly", Dragonfly },
			{"eagle", Eagle },
			{"electric_eel", ElectricEel },
			{"elephant", Elephant },
			{"fire_fly", Firefly },
			{"garden_spider", GardenSpider },
			{"garfish", Garfish },
			{"giant_anteater", GiantAnteater },
			{"giant_pacific_octopus", GiantPacificOctopus },
			{"colossal_squid", GiantSquid },
			{"giraffe", Giraffe },
			{"gorilla", Gorilla },
			{"great_white_shark", GreatWhiteShark },
			{"grizzly_bear", GrizzlyBear },
			{"hammerhead_shark", HammerheadShark },
			{"hatchetfish", Hatchetfish },
			{"hercules_beetle", HerculesBeetle },
			{"hippopotamus", Hippopotamus },
			{"hornbill", Hornbill },
			{"hornet", Hornet },
			{"horse",  Horse},
			{"humpback_whale", HumpbackWhale },
			{"hyena", Hyena },
			{"kangaroo", Kangaroo },
			{"killer_whale", KillerWhale },
			{"komodo_dragon", KomodoDragon },
			{"lammergeier", Lammergeier },
			{"lemming", Lemming },
			{"lioness", Lioness },
			{"lobster", Lobster },
			{"tapir", MalayanTapir },
			{"siphonophore", ManOWar },
			{"mantis_shrimp", MantisShrimp },
			{"moose", Moose },
			{"mountain_lion", MountainLion },
			{"musk_ox", MuskOx },
			{"mole_rat", NakedMoleRat },
			{"narwhal", Narwhal },
			{"oogpister", Oogpister },
			{"ostrich", Ostrich },
			{"panther", Panther },
			{"piranha", Piranha },
			{"pistol_shrimp", PistolShrimp },
			{"poison_frog", PoisonFrog },
			{"polar_bear", PolarBear },
			{"porcupine", Porcupine },
			{"praying_mantis", PrayingMantis },
			{"ram", Ram },
			{"rat", Rat },
			{"rattlesnake", Rattlesnake },
			{"raven", Raven },
			{"rhinoceros", Rhinoceros },
			{"scorpion", Scorpion },
			{"stink_bug", ShieldBug },
			{"skipper_caterpillar", SkipperCaterpillar },
			{"skunk", Skunk },
			{"snail", Snail },
			{"snapping_turtle", SnappingTurtle },
			{"snowy_owl", SnowyOwl },
			{"sperm_whale", SpermWhale },
			{"spitting_cobra", SpittingCobra },
			{"spitting_spider", SpittingSpider },
			{"tarantula", Tarantula },
			{"termite", Termite },
			{"tiger", Tiger },
			{"tuna", BluefinTuna },
			{"velvet_worm", VelvetWorm },
			{"vulture", Vulture },
			{"stick_bug", WalkingStick },
			{"walrus", Walrus },
			{"warthog", Warthog },
			{"spider_wasp", Wasp },
			{"whale_shark", WhaleShark },
			{"wolf", Wolf },
			{"wolverine", Wolverine },
			{"woolly_mammoth", WoollyMammoth },
			{"zebra", Zebra }
		};

		public static string[] ClawedArachnids =
		{
			Lobster,
			PistolShrimp,
			Scorpion,
			PrayingMantis,
			Tarantula,
			ManOWar,
			MantisShrimp
		};
	}
}
