...
namespace Combiner
{
    public class Database
    {
        ...
        public bool CreateModTable(LiteDatabase db)
        {
        ...
        public bool CreateMod(string name, string attrPath, string stockPath)
        {
            ...
            using (var db = new LiteDatabase(DirectoryConstants.DatabaseString))
            {
                ...
                CreateDB(db, tableName, attrPath, stockPath);
                return true;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<Creature> GetCreatureQuery(BsonExpression query, ModCollection mod)
        {
            using (var db = new LiteDatabase(DirectoryConstants.DatabaseString))
            {
                if (!db.CollectionExists(mod.CollectionName))
        ...
        public void UnsaveCreature(Creature creature, ModCollection modCollection)
        {
            using (var db = new LiteDatabase(DirectoryConstants.DatabaseString))
            {
                ...
                var collection = db.GetCollection<Creature>(modCollection.CollectionName);
                collection.DeleteMany(FindCreatureQuery(creature.Left, creature.Right, creature.BodyParts));
            }
        }

        private BsonExpression FindCreatureQuery(string left, string right, Dictionary<string, string> bodyParts)
        {
            return
                Query.And(
                    Query.Or(
                        ...
                        Query.Or(
                            Query.EQ("Left", right),
                            Query.EQ("Right", left))),
                    HasSameBodyParts(bodyParts));
        }

        // Build a list of BsonExpressions to check every limb for a match
        private BsonExpression HasSameBodyParts(Dictionary<string, string> bodyParts)
        {
            var partQueryList = new List<BsonExpression>();
            foreach (Limb limb in Enum.GetValues(typeof(Limb)))
            {
                ...
                partQueryList.Add(Query.EQ("$." + limb.ToString(), bodyParts[limb.ToString()]));
            }

            return Query.And(partQueryList.ToArray());
        }
        ...
        /// </summary>
        public void CreateDB(LiteDatabase db, string name, string attrPath, string stockPath)
        {
            Directory.CreateDirectory(DirectoryConstants.DatabaseDirectory);
        ...
        private void CreateCreatures(ILiteCollection<Creature> collection, string attrPath, string stockPath)
        {
